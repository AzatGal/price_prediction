...
class CatEmbedding(nn.Module):
    def __init__(self,
                 features_vocab_sizes: List[int],
                 embed_dim: int,
                 padding_idx: int = 0) -> None:
        super().__init__()
        self.weight = nn.Parameter(
            torch.empty(sum(features_vocab_sizes), embed_dim)
        )
        self.bias = nn.Parameter(
            torch.zeros(len(features_vocab_sizes), embed_dim)
        )
        self.padding_idx = padding_idx
        self.offsets = torch.tensor([0.0] + features_vocab_sizes[1:])
