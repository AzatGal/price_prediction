...
from models.modules.embedding import FeatureTokenizerEnsemble
from models.modules.block import TransformerEnsembleBlock
...
#                  ) -> None:
#         super().__init__()
#         self.embed = FeatureTokenizer(num_embed_features, embed_dim, dropout, False)
#         self.seq_len = self.embed.seq_len
#
#         self.mask_first_token = mask_first_token
#         if mask_first_token:
...
#     def reset_parameters(self) -> None:
#         for pn, p in self.named_parameters():
#             if 'norm' not in pn or 'comp' not in pn:
#                 if 'bias' in pn:
#                     nn.init.zeros_(p)
#                 elif 'head' in pn:
#                     nn.init.kaiming_uniform_(p, a=math.sqrt(5))
...
class TransformerEnsemble(nn.Module):
    def __init__(self,
        ...
        self.k = k
        self.add_cls_token = add_cls_token
        self.embed = FeatureTokenizerEnsemble(embed_dim // 2, n_embed_num, n_embed_cat,
        ...
        # self.embed_mlp = GatedMLPEnsemble(embed_dim, 2, k, act, dropout, True)
    ...
    def forward(self, x_num: torch.Tensor, x_cat: torch.Tensor) -> torch.Tensor:
        ...
        x = x.repeat(1, 1, 1, 2) * self.embed_rank
        # x = self.embed_proj(x)
        # x = x * self.embed_s
